...
def extract_visual_manifest(doc, image_dir: Path):
    ...
    return sorted(records, key=lambda x: (x.get("page") or 0, x["picture_id"]))


def build_canonical_document(doc, visual_results, docling_dict=None):
    visual_by_id = {
        x["picture_id"]: x for x in visual_results if x.get("status") == "success"
    }
    ...
    # Map picture object identity/order to notebook-style picture IDs.
    picture_id_by_ref = {}
    for idx, picture in enumerate(doc.pictures, start=1):
        try:
    ...
    for position, (item, _level) in enumerate(doc.iterate_items()):
        ...
        # Docling item classes are deliberately detected by attributes/type.
        ...
        if "table" in name:
            # Exported dict is used for robust table reconstruction.
            raw = tables_by_ref.get(getattr(item, "self_ref", None), {})
            elements.append({
                "element_id": f"element_{len(elements)+1:06d}",
                "type": "table",
                "position": position,
                ...
                "source": {"docling_ref": getattr(item, "self_ref", None)},
            })
            continue
...
def is_probable_page_number(text):
    ...
    return bool(re.fullmatch(r"\d{1,4}", text) or re.fullmatch(r"[ivxlcdmIVXLCDM]{1,8}", text))
...
def split_sentences(text):
    ...
    for paragraph in paragraphs:
        parts = re.split(r"(?<=[.!?])\s+(?=[A-Z₹0-9(\[])", paragraph.strip())
...
def build_chunks(canonical):
    elements = [
        e for e in canonical["elements"]
        if not (e["type"] == "text" and e.get("label") in {"page_header", "page_footer"})
    ]
    ...
    def flush():
        ...
        text = current_text()
        if not text:
            current_parts, current_ids, current_types, current_pages = [], [], [], []
            return
        chunks.append({
    ...
    for chunk in chunks:
        if len(chunk["content"]) >= MIN_STANDALONE_CHARS:
            ...
            continue
        if cleaned and len(cleaned[-1]["content"]) + 2 + len(chunk["content"]) <= MAX_CHARS:
            prev = cleaned[-1]
            prev["content"] += "\n\n" + chunk["content"]
...
def process_pdf(pdf_path: Path, work_dir: Path, api_key: str, progress_callback=None):
    ...
    if progress_callback:
        progress_callback("✓ PDF loaded — starting Docling extraction", 0.05)
    ...
    if progress_callback:
        progress_callback("✓ Content extracted with Docling", 0.35)

    visual_manifest = extract_visual_manifest(doc, image_dir)

    if progress_callback:
        progress_callback(f"✓ Visual candidates extracted ({len(visual_manifest)})", 0.45)

    visual_results = generate_visual_descriptions(
        visual_manifest, visual_json, api_key
    ) if visual_manifest else []

    if progress_callback:
        progress_callback("✓ Visual understanding completed", 0.60)

    docling_dict = result.document.export_to_dict()
    canonical = build_canonical_document(doc, visual_results, docling_dict)

    if progress_callback:
        progress_callback("✓ Canonical document created", 0.70)

    chunks = build_chunks(canonical)

    if progress_callback:
        progress_callback(f"✓ Document chunked ({len(chunks):,} chunks)", 0.80)
