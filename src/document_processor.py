...
if not cells:
    return clean_text(
        table.get("text") or table.get("content") or ""
    )
...
for idx, picture in enumerate(doc.pictures, start=1):
    ...
    try:
        ...
        width = (
            float(bbox.get("width", image.width))
            if bbox
            else float(image.width)
        )

        height = (
            float(bbox.get("height", image.height))
            if bbox
...
for position, (item, _level) in enumerate(
    doc.iterate_items()
):
    ...
    if "picture" in name:
        ...
        try:
            picture_id = picture_id_by_ref.get(
                item.self_ref
            )
        ...
        if (
            picture_id
            and picture_id in visual_by_id
        ):
...
for element in elements:
    ...
    if (
        typ != "text"
        or not text
        or is_probable_page_number(text)
    ):
        continue
...
# =============================================================
#
# Streamlit Cloud's Python site-packages directory is not
# writable. RapidOCR may otherwise try to download its model
# into:
#
# /home/adminuser/venv/lib/python3.14/site-packages/rapidocr/
#
# Instead, explicitly give Docling a writable model-artifact
...
# Keep the original project functionality.
...
if progress_callback:
    ...
    progress_callback(
        f"✓ Visual candidates extracted "
        f"({len(visual_manifest)})",
        0.45
    )
