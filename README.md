...
- `packages.txt` — system (apt) packages for Streamlit Community Cloud. Put
  this file at the **repo root** next to `requirements.txt` — Streamlit
  Cloud reads it automatically and apt-installs these before your app
  starts. Without it you'll hit `ImportError: libGL.so.1: cannot open
  shared object file`, since Docling's OCR/layout stack imports
  `opencv-python`, which needs that system library.

By default the app disables OCR (`do_ocr=False`) since most PDFs already
have a text layer and don't need it — this also means you can skip
`packages.txt` entirely for plain text PDFs. Turn on **"Enable OCR for
scanned pages"** in the sidebar's Advanced settings only if you're feeding
it scanned/image-only PDFs, and make sure `packages.txt` is deployed in
that case.
