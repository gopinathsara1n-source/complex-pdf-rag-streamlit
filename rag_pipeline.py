...
def _prepare_rapidocr_paths() -> dict:
    """
    Prepare writable paths for RapidOCR model files.

    RapidOCR will download the model files into these locations
    when they are not already present.
    """
    RAPIDOCR_MODEL_DIR.mkdir(parents=True, exist_ok=True)

    return {
        "det": RAPIDOCR_MODEL_DIR / "PP-OCRv6_det_small.pth",
        "rec": RAPIDOCR_MODEL_DIR / "PP-OCRv6_rec_small.pth",
        "cls": RAPIDOCR_MODEL_DIR / "ch_ptocr_mobile_v2.0_cls_mobile.pth",
        "dict": RAPIDOCR_MODEL_DIR / "ppocrv6_dict.txt",
    }
...
def build_converter(do_ocr: bool = False) -> DocumentConverter:
    ...
    rapidocr_options = RapidOcrOptions(
        backend="torch",
        rapidocr_params={
            "Det.model_path": str(rapidocr_paths["det"]),
            "Cls.model_path": str(rapidocr_paths["cls"]),
            "Rec.model_path": str(rapidocr_paths["rec"]),
            "Rec.keys_path": str(rapidocr_paths["dict"]),
        },
    )
...
def ingest_pdf(
    ...
    _tick(
        progress_cb,
        0.02,
        "Reading PDF with Docling (text, tables, images)...",
    )
    ...
    for attempt in range(
        ...
        try:
            ...
            if (
                ...
                delay = min(
                    VISION_INITIAL_RETRY_DELAY *
                    (2 ** attempt)
                    +
                    random.uniform(0, 1),
                    VISION_MAX_RETRY_DELAY,
                )
...
def describe_visuals(
    ...
    for n, item in enumerate(
        ...
        if (
            ...
            raise RuntimeError(
                "Gemini API key was rejected while "
                "describing visuals. Check your API key."
            )

        if n < len(visual_manifest):
            time.sleep(
                VISION_DELAY_BETWEEN_REQUESTS
            )
...
def build_canonical_elements(
    ...
    for position, child in enumerate(
        ...
        if item_type == "text":
            ...
            canonical_elements.append(
                {
                    "element_id": (
                        f"element_"
                        f"{len(canonical_elements)+1:06d}"
                    ),
                    "type": "text",
                    "position": position,
                    "page": page_no,
            ...
            picture_ref = item.get(
                "self_ref"
            )

            picture_id = (
                picture_ref_to_id.get(
                    picture_ref
                )
            )

            gemini_result = (
                gemini_by_picture.get(
                    picture_id
                )
            )
...
def chunk_elements(
    ...
    def add_element(
        ...
        current_element_ids.append(
            element["element_id"]
        )

        current_element_types.append(
            element["type"]
        )
...
def _embedding_error_is_retryable(
    exc
) -> bool:
...
def embed_chunks(
    ...
    output_dim=EMBEDDING_OUTPUT_DIMENSIONALITY,
    progress_cb: ProgressCB = None,
):

    texts = [
        _prepare_document(chunk)
        for chunk in chunks
    ]
    ...
    def wait_for_slot():
        ...
        if (
            elapsed
            <
            GEMINI_EMBEDDING_MIN_REQUEST_INTERVAL
        ):
    ...
    def embed_one(
        ...
        for attempt in range(
            ...
            try:
                ...
                if (
                    not _embedding_error_is_retryable(
                        exc
                    )
                    or attempt
                    ==
    ...
    for batch_start in range(
        ...
        if (
            ...
            time.sleep(
                GEMINI_EMBEDDING_INTER_BATCH_DELAY
            )
        ...
        results.append(
            {
                ...
                "metadata": doc_index.metadata[idx],
            }
        )
...
def generate_answer(
    ...
    if chat_history:
        ...
        history_block = (
            "\nRECENT CONVERSATION "
            "(for follow-up context only — "
            "the document context above is still "
...
def process_pdf(
    ...
    gemini_visuals = describe_visuals(
        client,
        visual_manifest,
        vision_models,
        progress_cb,
    ...
    stats = {
        ...
        "visuals_described": sum(
            1
            for g in gemini_visuals
            if g.get("status")
            == "success"
        ),
        ...
        "text_elements": element_stats.get(
            "text",
            0
        ),

        "table_elements": element_stats.get(
            "table",
            0
        ),

        "visual_elements": element_stats.get(
            "visual",
            0
        ),
    }
